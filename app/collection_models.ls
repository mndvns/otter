...
MIX = {}=
  ...
  Cite : {}=
    cite-proc    : ({field, list, attr}, [compare, [cursor, method], query = {}, options = {}] ) ->

      com = _.extend {([compare.0]) : @[compare.1]}, query
      col = ENV[list.to-proper-case!][cursor]( com, options )

      col = col[that]! if method?
    ...
    cite-jam     : -> 
      for p in @@@_cites => @cite-proc p[0], p[1], p[2], p[3]
      @
...
GENERATE = ([K, C, S, T, F, M]) ->

  object = ^^K.klass
  class object extends Model

  switch
  | (C.scratch or C.stable)?  => Coll = null
  ...
  if C.cache?       => for c in C.cache  =>  Klass[c.coll] = Klass{}._cache[c.coll] = new MC null, { id-generation: \STRING, transform: c.trans }
  if C.stable?      => for s in C.stable =>  Klass._collection.insert (_.extend s, C.uniform)
  ...
  if T && IS-CLIENT => for t, v of T         => ENV.Template[K.klass.to-string!to-lower-case!][t] = v
  ...
  EJSON.add-type Klass._type.model, Klass.new!
    ...
    method :
      ...
      load-store : ->
        Deps.autorun ~>
          ...
          switch
          | Is.mine @get-store! => (console.log 'LOAD!', 'IS MINE'     )
          | @mine!count! > 0    => (console.log 'LOAD!', 'LOADING MINE') and ( My.offer! ..store-load!            )
          | _                   => (console.log 'LOAD!', 'DEFAULT SET' ) and ( @new! ..default-set! ..store-load! )
...
if IS-CLIENT? => Session.set 'loaded_collections', true
